...
**Interpretation:**
  ...
  - Model: the `model2_2` I created is a simple linear regression, with
    gestational age as predictor (X) and birthweight as Y
  - Plot: from the plot, since the residuals are relatively evenly
    distributed around and below y = 0. Thus, it satisfied the linear
    assumption.
...
**Interpretation:** The first model have the highest rmse, and the third
model have the lowest rmse. Thus, the last one have the lowest
prediction error, which means that the model with the predictors of head
circumference, length, sex, and all interactions (including the
three-way interaction) between these is the best among the three models.
...
simple linear regression:
  ...
  - interested in the distribution of two quantities estimated from
    these data:
      - \({\widehat r ^2}\)
...
```
    ...
    ## # A tibble: 5,000 x 3
    ##    strap_number r.squared variable2
    ##           <int>     <dbl>     <dbl>
    ##  1            1     0.921      1.99
    ##  2            2     0.899      2.04
    ##  3            3     0.911      2.03
...
![](p8105_hw6_yf2563_files/figure-gfm/unnamed-chunk-22-1.png)<!-- -->

**Interpretation:** both plots follows bell-shaped, which means that
both variables we estimated are approximate follows normal distribution

  - Using the 5000 bootstrap estimates, identify the 2.5% and 97.5%
    quantiles to provide a 95% confidence interval for 2 interested
    variables.
...
```
    ...
    ## # A tibble: 2 x 3
    ##   variables ci_lower ci_upper
    ##   <chr>        <dbl>    <dbl>
    ## 1 r.squared    0.894    0.927
    ## 2 variable2    1.97     2.06

Thus 2.5% and 97.5% quantiles:

  - \(\widehat r ^2\): (0.8879076, 0.925854)
  - \({log(\widehat \beta_0 * \widehat \beta_1)}\): (1.9729733,
    2.052170)
